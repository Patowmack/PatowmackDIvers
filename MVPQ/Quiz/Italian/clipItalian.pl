...
# program will create records in G:\XAMPP\xampplite\apache\htdocs\Quiz\quizItalian.sqlite3 (see $db)
#
...
my $db = DBI->connect("dbi:SQLite:G:/XAMPP/xampplite/apache/htdocs/Quiz/quizItalian.sqlite3", "", "", {RaiseError => 1, AutoCommit => 1});
#insert
   my $ith = $db->prepare(<<SQL) || die "SQL error:" . DBI->errstr;
   INSERT INTO quiz ("group", xOther, english, language) VALUES (?, ?, ?, "Italian")
SQL
